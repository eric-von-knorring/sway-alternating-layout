...
fn set_layout(focused: Node, command: &mut Connection) {
   if let Ok(Some(parent)) = command.get_tree().map(|tree| tree.find(|node| node.nodes.contains(&focused))) {
    ...
    if focused_current.rect.height > focused_current.rect.width {
        if let Err(error) = command.run_command("splitv") {
            eprintln!("Error: Failed to run splitv. {}", error)
        };
    } else {
        if let Err(error) = command.run_command("splith") {
